...
/* Returns true if e1 is less than e2. Comparison used for insert_ordered (priority-queue) */
static bool alarm_compare(const struct list_elem *e1, const struct list_elem *e2, void *aux UNUSED) {
...
void timer_sleep(int64_t ticks) {
  ...
  enum intr_level old_level = intr_disable();
  list_insert_ordered(&sleeping_threads, &t->elem, alarm_compare, NULL);
  thread_block();
  intr_set_level(old_level);
}
...
static void timer_interrupt(struct intr_frame* args UNUSED) {
  ...
  int current_priority = thread_current()->e_priority;

  for (struct list_elem *e = list_begin(&sleeping_threads); e != list_end(&sleeping_threads);) {
    struct thread *t = list_entry(e, struct thread, elem);

    if (t->alarm_time > ticks) break;

    if (!yield_thread && t->e_priority > current_priority) {
      yield_thread = true;
    }

    struct list_elem *rm = e;
